...
class UI_Element:
    ...
    def dirty_size(self):
        ...
        self.dirty_count += 1
        for cb in self.dirty_callbacks: cb()
        self._dirty_size()

    def register_dirty_callback(self, fn):
        if fn not in self.dirty_callbacks:
            self.dirty_callbacks.append(fn)

    def unregister_dirty_callback(self, fn):
        self.dirty_callbacks = [cb for cb in self.dirty_callbacks if cb != fn]

    def get_width(self):
        if self.cached_width is None:
            self.cached_width = (self._get_width() + self.drawing.scale(self.margin*2)) if self.visible else 0
        return self.cached_width

    def get_height(self):
        if self.cached_height is None:
            self.cached_height = (self._get_height() + self.drawing.scale(self.margin*2)) if self.visible else 0
...
class UI_Container(UI_Element):
    ...
    def update_widths(self):
        self.ui_widths = [ui.get_width() for ui in self.ui_items]

    def update_heights(self):
        self.ui_heights = [ui.get_height() for ui in self.ui_items]

    def _get_width(self):
        self.update_widths()
        if not self.ui_items: return 0
        fn = max if self.vertical else sum
        return fn(self.ui_widths)

    def _get_height(self):
        self.update_heights()
        if not self.ui_items: return 0
        fn = sum if self.vertical else max
        return fn(self.ui_heights)
    ...
    def _draw(self):
        ...
        if self.vertical:
            y = t
            for ui,eh in zip(self.ui_items, self.ui_heights):
                ui.draw(l,y,w,eh)
                y -= eh
        else:
            x = l
            ...
            for i in range(l):
                ui = self.ui_items[i]
                ew = self.ui_widths[i] if i < l-1 else w
                ui.draw(x,t,ew,h)
                x += ew
                w -= ew
    ...
    def add(self, ui_item, only=False):
        if only:
            for ui in self.ui_items:
                ui.unregister_dirty_callback(self.dirty_size)
            self.ui_items.clear()
        self.ui_items.append(ui_item)
        ui_item.register_dirty_callback(self.dirty_size)
        self.dirty_size()
        return ui_item

    def register_with_children(self):
        for ui_item in self.ui_items:
            ui_item.register_dirty_callback(self.dirty_size)
...
class UI_Image(UI_Element):
    ...
    def set_width(self, w):
        self.width,self.size_set = w,True
        self.dirty_size()
    def set_height(self, h):
        self.height,self.size_set = h,True
        self.dirty_size()
    def set_size(self, w, h):
        self.width,self.height,self.size_set = w,h,True
        self.dirty_size()
...
class UI_IntValue(UI_Container):
    def __init__(self, label, fn_get_value, fn_set_value, fn_get_print_value=None, fn_set_print_value=None, margin=4, bgcolor=None, hovercolor=(1,1,1,0.1), presscolor=(0,0,0,0.2), **kwargs):
        ...
        # self.margin = 0
        self.lbl = UI_Label(label)
        self.val = UI_Label((fn_get_print_value if fn_get_print_value else fn_get_value)())
        self.add(self.lbl)
        self.add(UI_Label(':'))
        self.add(UI_Spacer(width=4))
        self.add(self.val)
...
class UI_Collapsible(UI_Container):
    def __init__(self, title, collapsed=True, fn_collapsed=None, equal=False, vertical=True):
        ...
        def update():
            self.last_collapsed = self.collapsed
        ...
        def set_collapsed(v):
            if fn_collapsed:
                ...
                self.collapsed = v
            if self.last_collapsed == self.collapsed: return
            update()
    ...
    def predraw(self):
        collapsed = self.fn_collapsed.get()
        #self.title.set_bgcolor(self.bgcolors[collapsed])
        self.title_arrow.set_graphic(self.graphics[collapsed])

    def _get_width(self):
        self.update_widths()
        return max(c.get_width() for c in self.ui_items if c.ui_items)

    def _get_height(self):
        self.update_heights()
        return sum(c.get_height() for c in self.ui_items if c.ui_items)
...
class UI_Padding(UI_Element):
    ...
    def set_ui_item(self, ui_item):
        if self.ui_item:
            self.ui_item.unregister_dirty_callback(self.dirty_size)
