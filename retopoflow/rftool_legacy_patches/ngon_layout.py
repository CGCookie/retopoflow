...
def cc_solve(counts, *, strict=False):
    ''' Every s solving e_i = s_{i-1} + s_{i+1} for the side counts, most balanced first; empty when
    the loop is not CC-able. '''
    n = len(counts)
    if n < 3 or sum(counts) % 2: return []
    lo = 1 if strict else 0

    def walk(start):
        # walking i -> i+2 with s_{i+2} = e_{i+1} - s_i makes every s_j on the chain a_j + b_j * k, b_j = +-1, in
        # the chain's starting unknown k; returns the chain and what s_start closes to. Closing either fixes k
        # (odd n, or n = 2 mod 4) or leaves it free inside an interval (n = 0 mod 4), which is Tarini's k
        lin = { start: (0, 1) }
        i = start
        while True:
            j = (i + 2) % n
...
def cc_hint(counts):
    ''' Smallest parity-preserving change to the side counts that would make the loop CC-able, as
    (side index, delta) pairs, or None. '''
    n = len(counts)
    # small edits first (two on one side, one each on two sides), then larger edits to a single side, which is
    # what an extreme loop needs
    tries = []
    for i in range(n):
        tries.append([(i, 2)])
    ...
    for edit in tries:
        ...
        if cc_state(tuple(c))[1]: return edit
    return None
...
def plan_shape(plan):
    ''' The kind, whether the pole is inside, and each piece's side counts up to rotation and
    reflection: plans sharing one are placements of one Solution, which Offset steps through. '''
    return (plan.kind, plan.strict, tuple(sorted(cycle_key(loop.counts()) for loop, _ in plan.pieces)))
...
def plan_phantom(loop):
    ''' Plans for a loop with an odd edge count, which no quad fill closes: one side carries an
    imaginary extra vertex, the loop is filled round a pole, and build_layout takes the vertex out
    again, leaving one non-quad. The mode says how:
      'ngon'  dissolves the vertex's spoke; the pole's two faces there become one pentagon (a
              triangle on a 3-sided loop, whose pole dissolves too) and the pole drops one valence
      'full'  collapses the strip beside the spoke, pole row included: a triangle beside the pole,
              which gains one valence
      'short' collapses that strip but stops one quad short: a triangle, the pole keeps its valence
              and the vertex beside it goes to five '''
    cs = loop.corners
    n = len(cs)
    counts = list(loop.counts())
    if sum(counts) % 2 == 0: return []
    # strict solutions only: with the pole on the boundary, taking the imaginary vertex out again loses a boundary
    # edge or leaves a face out. 'ngon' ranks first, then 'full' up to four sides, where the pole stays at five or
    # less, else 'short'
    plans = []
    for j in range(n):
        padded = list(counts)
...
def plan_phantom_merges(loop, sharpness=None):
    ''' plan_phantom's plans for the loops left by demoting corners, for an odd loop none of whose
    own sides can carry the imaginary vertex. sharpness maps a corner position to how sharply the
    boundary turns there: the plans demoting the least sharp corners rank first, then fewer
    demotions, then plan_phantom's order. '''
    cs = loop.corners
    n = len(cs)
    plans = []
    # a four-sided loop has a single pole only with opposite sides equal, so 2,3,4,4 gets nothing on its own; with
    # the corner between the 2 and the 3 demoted it is the 5,4,4 triangle, which has one
    for r in range(1, n - 2):
        for demote in itertools.combinations(range(n), r):
            kept = tuple(cs[k] for k in range(n) if k not in demote)
            merged = Loop(loop.nodes, kept)
...
def phantom_shape(plan):
    ''' plan_shape for an odd loop's plans: the counts of the loop filled, demoted corners merged and
    the imaginary vertex taken out, up to rotation and reflection, with the mode. '''
    piece, _ = plan.pieces[0]
    _, j, mode = plan.phantom
    counts = list(piece.counts())
    counts[j] -= 1
    return cycle_key(counts), mode


def cut_loop(loop, p, q, cut_nodes):
...
def cut_score(pieces, irregular):
    boundary_poles = sum(1 for _, s in pieces if min(s) < 1)
    return (len(pieces), irregular, boundary_poles, 0)


def plan_cuts(loop, *, max_depth=2, limit=8, cut_id=0, base_corners=None):
    ''' Plans that cut the loop into CC-able pieces along new edge runs, a cut that ends on a corner
    ranking after one between two plain verts, which keeps every corner regular. '''
    m, cs = len(loop.nodes), loop.corners
    if base_corners is None: base_corners = { loop.nodes[c] for c in cs }
    corner_set = set(cs)
    ...
    # depth 1 is exhaustive over pairs of boundary positions; depth 2 only recurses into the piece that stopped
    # the best depth-1 attempts
    plans, partial = [], []
    for p in range(m):
        for q in range(p + 1, m):
            ...
            irregular = (loop.nodes[p] in base_corners) + (loop.nodes[q] in base_corners)
            ...
            for x in range(xmin, min(other_a, other_b) + 1, 2):
                ok_a, ok_b = cc_state(tuple(base_a + [x])), cc_state(tuple(base_b + [x]))
                if not (ok_a[1] or ok_b[1]): continue
                if not (ok_a[1] and ok_b[1]) and max_depth <= 1: continue
                cut = (loop.nodes[p],) + tuple(('cut', cut_id, k) for k in range(1, x)) + (loop.nodes[q],)
                a, b = cut_loop(loop, p, q, cut)
                if ok_a[1] and ok_b[1]:
                    pieces = [(a, solve(list(a.counts()))), (b, solve(list(b.counts())))]
                    plans.append(Plan('cut', pieces, cuts=[cut], score=cut_score(pieces, irregular)))
                else:
    ...
    for irregular, good, bad, cut in partial[:6]:
        for sub in plan_cuts(bad, max_depth=max_depth - 1, limit=2, cut_id=cut_id + 1 + len(plans),
                             base_corners=base_corners):
            pieces = [(good, solve(list(good.counts())))] + sub.pieces
            plans.append(Plan('cut', pieces, cuts=[cut] + sub.cuts,
                              score=cut_score(pieces, irregular + sub.score[1])))
    plans.sort(key=lambda p: p.score)
    return plans[:limit]
...
KEY_RANK = { 'cut': 1, 'spoke': 2, 'pole': 3, 'int': 4 }     # which key names a node unified with others: the loop's own first

def key_rank(key):
    return KEY_RANK.get(key[0], 4) if isinstance(key, tuple) and key and isinstance(key[0], str) else 0
...
def dissolve_run(faces, run):
    ''' Dissolve a node run's edges and the two-valent nodes that leaves, as Blender's dissolve edge
    loop does: (the faces left, the nodes no face uses any more). '''
    ...
    gone = set(run[:-1])
    if sum(1 for f in faces if run[-1] in f) <= 2: gone.add(run[-1])     # a far end left with two faces: a 3-sided loop's pole
...
def build_layout(plan):
    ...
    def union(a, b):
        ra, rb = find(a), find(b)
        if ra == rb: return
        if (key_rank(ra), repr(ra)) <= (key_rank(rb), repr(rb)): parent[rb] = ra
        else: parent[ra] = rb

    raw_faces, raw_regions, raw_lines, raw_poles, existing = [], [], [], [], set()
    spoke_to_dissolve, raw_helpers = None, []
    ...
    unused = set()
    if spoke_to_dissolve is not None:
        faces, unused = dissolve_run(faces, [idx(k) for k in spoke_to_dissolve])
...
def straight_through(layout, loop, bad=None):
    ''' How many quads run along two consecutive boundary edges at one of the loop's `bad` positions,
    by default every vert that is not a corner: the boundary passes straight through a corner of
    theirs, so on the mesh they read as triangles; at a reflex corner they are concave. '''
    m = len(loop.nodes)
...
def layout_key(layout):
    ''' What a layout is as a mesh on its loop, so two layouts making the same quads between the same
    boundary verts get the same key however they were built; a turnout's two layings share one. '''
    # the loop's own verts are named by key and every other vert by the names round it, refined until the names
    # stop splitting (Weisfeiler-Lehman); names are compressed to small ints each round or they would double in size
    around = {}
...
def step_fits(counts):
    ''' For a four-sided loop with one pair of opposite sides equal and the other an even number
    apart: (r, m, n, d), the rotation putting the equal rails at sides r and r + 2 and the shorter
    of the others at r + 3, the rails' m edges, that side's n and the half difference d. Else None. '''
...
def junctions(m, n, d):
    ''' The junctions taking up a step of 2 d between rails of m edges from a side of n, in the
    order to offer them. '''
    # the bow fits anywhere and takes up any step, one bow nested in the next, at the price of two 3-poles. The
    # diamond, one 5-pole and one 3-pole, takes up a step of two and needs a rail and a short side of two to sit
    # inside the loop; it leads except when n is odd, where the bow is symmetric about the middle row and the
    # diamond sits half a row off it
    kinds = ['bow']
...
def diamond_positions(m, n):
    ''' Every (k, j) a diamond junction can take between rails of m edges with n on the shorter side,
    its 5-pole vert j up column k and its 3-pole half a column past column k + 1: most central first,
    then fanning outward. '''
...
def rail_columns(sides, height):
    ''' (nodes, idx, columns, existing) for a four-sided loop laid over a rectangle: node indices up
    every column c, height(c) of them, the sides' own keys where a column meets them. sv0 and sv2
    are the rails the columns cross between, sv3 is column 0 and sv1 column m: sv0[0] == sv3[0],
    sv0[-1] == sv1[0], sv2[0] == sv3[-1] and sv2[-1] == sv1[-1]. '''
...
def add_blocks(faces, regions, grids):
    ''' The quads of regular runs of columns, grid[u][v] with every column the same height, wound
    like the loop, each run one region. '''
    for grid in grids:
...
def junction_layout(nodes, faces, regions, existing):
    ''' A Layout with no pole, spoke or cut: a junction's verts are all placed by its caller. '''
    edges = sorted({ (min(a, b), max(a, b)) for f in faces for a, b in zip(f, f[1:] + f[:1]) })
    return Layout(nodes, faces, edges, regions, [], [], [], existing, set(), [])


def build_diamond(sides, k, j):
    ''' Layout of a four-sided loop as columns of quads between equal rails sv0 and sv2, a side sv3
    of n edges and a side sv1 of n + 2 (rail_columns' orientation), a diamond junction taking up the
    step with its 5-pole at vert j up column k. Returns (Layout, columns, R): columns[c][r] the node
    of vert r up column c and R the 3-pole, all of which the caller places. '''
    n = len(sides[3]) - 1
    m = len(sides[0]) - 1
    # columns hold n + 1 verts through column k, n + 2 on column k + 1 and n + 3 from column k + 2 on
    nodes, idx, columns, existing = rail_columns(sides, lambda c: n + 1 if c <= k else n + 2 if c == k + 1 else n + 3)
    R = idx(('pole', 0))
    faces, regions = [], []
    add_blocks(faces, regions, [
    ...
    # the long side's lines j and j + 2 reach column k + 1 as T and B and run on into the 5-pole L; its line j + 1
    # stops at the 3-pole R between columns k + 1 and k + 2. [T, R, B, L] is the diamond, wound with the loop
    T, B, L = columns[k + 1][j], columns[k + 1][j + 1], columns[k][j]
    faces.append((T, R, B, L))
    ...
    return junction_layout(nodes, faces, regions, existing), columns, R


def build_bow(sides, k, depth=1):
    ''' Layout of a four-sided loop as columns of quads between equal rails sv0 and sv2, a side sv3
    of n edges and a side sv1 of n + 2 depth (rail_columns' orientation), depth nested bows through
    column k + 1 taking up the step. Returns (Layout, columns, bows): columns[c][r] the node of vert
    r up column c, bows[t][i] the vert of bow t (outermost first) beside column k's vert i, all of
    which the caller places. '''
    n = len(sides[3]) - 1
    m = len(sides[0]) - 1
    long = n + 2 * depth
    nodes, idx, columns, existing = rail_columns(sides, lambda c: n + 1 if c <= k else long + 1)
    ...
    # each line of n + 1 verts leaves column k's end verts and bows through column k + 1, one inside the next; from
    # the outside in, bow t's ends pair with column k + 1's verts t and long - t under one fan quad each, so the long
    # side's outer lines end on the bows and only the innermost bow's ends are 3-poles
    B = columns[k + 1]
    lines = [columns[k]] + bows
    ...
    return junction_layout(nodes, faces, regions, existing), columns, bows
...
def orient_sides(sides, r, reflected=False):
    ''' A loop's four sides in rail_columns' orientation: side r as sv0 (the top rail), r + 1 as sv1
    (the right side), r + 2 reversed as sv2 (the bottom rail) and r + 3 reversed as sv3 (the left
    side), the loop first run the other way round when `reflected`. Every one of the eight ways of
    laying the rectangle is some (r, reflected). '''
...
def turnout_fits(counts):
    ''' Every way a four-sided loop takes a turnout, as (r, reflected) for orient_sides: the left side
    sv3 has d >= 1 more edges than the right side sv1 and the bottom rail sv2 has d more than the top
    rail sv0. The ways leaving through the longer rail come first. A loop that fits both ways round
    gives two layings, but they are one mesh seen from two sides (layout_key agrees), so the first is
    all a caller needs. '''
    if len(counts) != 4: return []
    fits = []
    # the top rail needs two or more edges for the poles' column to have a block on each side
...
def turnout_shape(counts, orient):
    ''' (base, short, d) of a turnout laying, the top rail's edges, the short side's and the step:
    layings sharing one are mirror images across the loop, one Solution. '''
    r, reflected = orient
    c = counts[::-1] if reflected else list(counts)
...
def turnout_positions(base, short, d):
    ...
    edges with d rows turning out, the poles on column k and the 5-pole on line j: most central first,
    a tie going to the column nearer the short side, where the rows converge. '''
    tall = short + d
    # j = 0 puts the 5-pole on the top rail, the only place when the short side is a single edge
...
def build_turnout(sides, k, j):
    ...
    whose bottom rail sv2 has d more than its top rail sv0 (orient_sides' orientation): the d extra
    rows leave through the bottom rail past a 5-pole P on line j of column k and a 3-pole Q on line
    j + d. Returns (Layout, left, right, wedge): left[c][r] the node up column c of the block at the
    left side's row count (left[k] the seam), right[c - k][r] the block at the right side's, wedge[u][v]
    the quads the rows leave across, u from P toward Q and v from the seam toward the right. The
    caller places the seam and the wedge's right side; the rest are region interiors. '''
    sv0, sv1, sv2, sv3 = sides
    ...
    # k columns at the left side's row count, base - k at the right side's, and between them the seam, column k's
    # line. The wedge is d x w, w the lines from Q to the bottom rail; its corners are P, Q, the seam's foot on the
    # bottom rail and the rail vert d further along, and its fourth side runs from that vert back up to P, which
    # with the seam above P is the right block's first column
    left = [ [ idx(sv3[r] if c == 0 else sv0[c] if r == 0 else sv2[c] if r == tall else ('col', c, r))
               for r in range(tall + 1) ] for c in range(k + 1) ]
    seam = left[k]
    right_edge = [ seam[r] if r <= j else idx(sv2[k + d]) if r == short else idx(('wedge', 0, r - j)) for r in range(short + 1) ]
    right = [ right_edge ] + [ [ idx(sv1[r] if c == base else sv0[c] if r == 0 else sv2[c + d] if r == short else ('rcol', c, r))
                                 for r in range(short + 1) ] for c in range(k + 1, base + 1) ]
    wedge = [ [ seam[j + u] if v == 0 else seam[j + d + v] if u == d else idx(sv2[k + d - u]) if v == w
                else right_edge[j + v] if u == 0 else idx(('wedge', u, v))
                for v in range(w + 1) ] for u in range(d + 1) ]
    existing = { idx(key) for sv in sides for key in sv }
    faces, regions = [], []
    add_blocks(faces, regions, [ left, right ])
    for u in range(d):
        for v in range(w):
            # wound the other way round from a block: the wedge's u runs down the seam, which the left block's rows run up
            faces.append((wedge[u][v], wedge[u][v + 1], wedge[u + 1][v + 1], wedge[u + 1][v]))
    regions.append(wedge)
    return junction_layout(nodes, faces, regions, existing), left, right, wedge
