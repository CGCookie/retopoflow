...
from ..common.bmesh import bme_other_bmv, bmes_shared_bmv, bmf_is_quad, bmf_midpoint, get_bmesh_emesh, NearestBMVert
...
def get_perimeter_bmedges(bmfaces):
    ...
    n0 = sum(( bmv.normal for bmv in perimeter_bmverts ), Vector((0,0,0)))
    n1 = sum(
        (
...
@execute_operator('rip_rotate_zip', 'RetopoFlow: Rip Rotate Zip')
def rip_rotate_zip(context):
    ITERATIONS = 1000
    SPRING_K = 10
    SPRING_C = 0.95
    TIMESTEP = 0.02
    ...
    matrix_world = context.edit_object.matrix_world
    ...
    for _ in range(ITERATIONS):
        ...
        for (bmv, f) in forces.items():
            bmv.co = bmv.co + TIMESTEP * vels[bmv]
            vels[bmv] = TIMESTEP * forces[bmv] - (1 - TIMESTEP * SPRING_C) * vels[bmv]
