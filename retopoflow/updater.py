...
try:
    from ..addon_common.common.updater_core import Updater as updater
...
class addon_updater_install_popup(bpy.types.Operator):
    ...
    bl_label = "Update RetopoFlow addon"
    bl_idname = "cgcookie.retopoflow_updater_install_popup"
...
class addon_updater_update_target(bpy.types.Operator):
    bl_label = "RetopoFlow version target"
    bl_idname = "cgcookie.retopoflow_updater_update_target"
    bl_description = "Install a targeted version of the RetopoFlow addon"
    bl_options = {'REGISTER', 'INTERNAL'}
...
class addon_updater_install_manually(bpy.types.Operator):
    ...
    bl_label = "Install RetopoFlow update manually"
    bl_idname = "cgcookie.retopoflow_updater_install_manually"
    bl_description = "Proceed to manually install RetopoFlow update"
    bl_options = {'REGISTER', 'INTERNAL'}
...
class addon_updater_updated_successful(bpy.types.Operator):
    ...
    bl_label = "RetopoFlow Installation Report"
    bl_idname = "cgcookie.retopoflow_updater_update_successful"
...
class addon_updater_restore_backup(bpy.types.Operator):
    ...
    bl_label = "Restore RetopoFlow Backup"
    bl_idname = "cgcookie.retopoflow_updater_restore_backup"
...
class addon_updater_ignore(bpy.types.Operator):
    ...
    bl_label = "Ignore RetopoFlow update"
    bl_idname = "cgcookie.retopoflow_updater_ignore"
...
class addon_updater_end_background(bpy.types.Operator):
    ...
    bl_idname = "cgcookie.retopoflow_end_background_check"
...
def updater_run_install_popup_handler(scene):
    ...
    #   return
        ...
        if ver_tuple < updater.current_version:
            ...
            if updater.verbose:
                print("RetopoFlow updater: appears user updated, clearing flag")
            updater.json_reset_restore()
            return
    atr = addon_updater_install_popup.bl_idname.split(".")
    getattr(getattr(bpy.ops, atr[0]),atr[1])('INVOKE_DEFAULT')
...
def post_update_callback(module_name, res=None):
    ...
    if res==None:
        ...
        if updater.verbose:
            print("RetopoFlow updater: Running post update callback")
...
def check_for_update_background():
    ...
    *Could* be called on register, but would be bad practice.
    ...
    # if false, no update ready
    if updater.verbose:
        print("RetopoFlow updater: Running background check for update")
...
def update_settings_ui(self, context, element=None):
    ...
    elif updater.update_ready and updater.manual_only==False:
        subcol = col.row(align=True)
        subcol.scale_y = 1
        split = subcol.split(align=True)
        split.scale_y = 2
