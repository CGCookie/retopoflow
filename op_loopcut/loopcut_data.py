...
class LoopCut(object):
    ...
    def find_face_loop(self,bme, ed, select = False):
        ...
        if len(loop_faces) == 2:        
            self.face_loop_fs = loop_faces[0] +  loop_faces[1].reverse()
            self.face_loop_eds = loop_eds[0] + loop_eds[1][1:].reverse()
        else:
    ...
    def cut_loop(self, bme, select = True):
        ...
        geom =  bmesh.ops.bisect_edges(self.bme, edges = eds,cuts = 1,edge_percents = ed_pcts)
        
        new_verts = [ele for ele in geom['geom_split'] if isinstance(ele, bmesh.types.BMVert)]
        
        print('connect vert pairs')
        new_edges = []
        for i in range(0,len(new_verts)):
            self.bme.verts.ensure_lookup_table()
            v_pair = [self.bme.verts[new_verts[i]], self.bme.verts[new_verts[i-1]]]
            ed_geom = bmesh.ops.connect_vert_pair(self.bme, verts = v_pair, verts_exclude = [], faces_exclude = [])
    ...
    def push_to_edit_mesh(self,bme):
        ...
        bme.update_edit_mesh(target_ob.data)
