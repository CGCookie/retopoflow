...
If you select an edge to start from and knife to another edge along the same face loop, PolyPen will insert a partial edge loop instead of a straight cut if **Knife Loops Cuts** is enabled. This allows you to quickly insert partial loops along curved surfaces.
...
The **Knife Constrain** option is useful for keeping cut edges on the actual cut. If you would prefer to freely move the resulting vert after the cut instead of sliding along the edge, you can turn this option off.
