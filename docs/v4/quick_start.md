...
All Retopoflow tools that create new geometry do so when you hold `Ctrl` (`Control` on Mac, not `Command`) and click or drag, while left clicking or dragging without `Ctrl` is for selection and tweaking.
