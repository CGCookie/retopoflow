...
ignore_exts = {'.ttf', '.exr', '.blend', '.pack', '.mtl', '.json', '.m4v', '.md', '.LICENSE', '.jpg', '.html', '.blend1', '.png', '.pyc', '.yml'}
unhandled_exts = set()
unhandled_paths = []


def process(p):
    global date, update_exts, unhandled_exts, unhandled_paths, ignore_exts, ignore_dirs
    if p.is_file():
        if p.suffix not in update_exts:
            ...
            return
        orig = p.read_text()
        updated = re.sub(r'Copyright \(C\) (?P<year>\d+)', f'Copyright (C) {date}', orig)
