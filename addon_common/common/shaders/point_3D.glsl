...
struct Options {
    mat4 MVPMatrix_view;   // view matrix
    mat4 MVPMatrix_pixel;  // pixel matrix
...
void main() {
    ...
    vec4 center = options.MVPMatrix_view * vec4(options.center.xyz, 1.0);
    vec2 p = center.xy + (pos - vec2(0.5, 0.5)) * radius_border;
    gl_Position = options.MVPMatrix_pixel * vec4(p, 0.0, 1.0);
...
void main() {
    ...
    vec2 ctr = (options.MVPMatrix_pixel * vec4(options.center.xy, 0.0, 1.0)).xy;
